...
def get_data(filters):
	...
	for emp in employees:
		salary_slips = frappe.db.sql("""select * from `tabSalary Slip` where employee = '%s' and docstatus != 2 and start_date between '%s' and '%s'  """%(emp.name,filters.from_date,filters.to_date),as_dict = True)
