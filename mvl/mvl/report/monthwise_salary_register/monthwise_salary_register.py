...
def execute(filters=None):
	...
	for ss in salary_slips:
		sum_emp = frappe.db.sql("""select sum(basic + house_rent_allowance + dearness_allowance + washing_allowance + conveyance_allowance + medical_allowance + special_allowance + other_allowance) as total from `tabEmployee` where employee='%s' """%(ss.employee),as_dict=1)[0]
		row = [
				ss.employee,
				ss.employee_name,
				...
				# frappe.db.get_value('Employee',ss.employee,'other_allowance') or 0,
				...
				# int(frappe.get_value('Salary Detail',{'salary_component':"Lunch Allowance",'parent':ss.name},["amount"]) or 0),
