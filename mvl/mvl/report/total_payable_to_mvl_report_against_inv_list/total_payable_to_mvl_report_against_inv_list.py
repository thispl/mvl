...
def get_data(filters):
	...
	for iv in invoice:
		frappe.errprint(iv.name)
		if iv.travel_allowance == 0:
			...
			row1 = [1,iv.inv_name, (salary_slip['amount'] or 0 )]
			data.append(row1)
		elif iv.travel_allowance == 1:
	...
	for i in invoice:
		if i.inv_name == "Renowned Engineers Private Limited,,Manpower Supply Service":
			...
			if type(lun_all['amount']) == float:
				...
				lun = 0
			row1 = [1,"Renowned Engineers Private Limited,,Manpower Supply Service Lunch",lun or 0]
			data.append(row1)
	return data